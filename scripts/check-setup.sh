...
for script in scripts/bootstrap-codex.sh scripts/llm-critic.sh scripts/check-setup.sh; do
  bash -n "$script"
done
...
memory_check="${KIT_HOME:-$HOME}/parallel-ai-dev/scripts/self-check.sh"
if [ ! -f "$memory_check" ]; then
  printf '%s\n' "Missing upstream memory checker: $memory_check" >&2
  exit 1
fi
bash "$memory_check"
