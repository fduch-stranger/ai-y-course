...
run_slot() {
  ...
  case "$backend" in
    ...
    codex-cli)
      ...
      cat "$pf" | codex exec --skip-git-repo-check --sandbox read-only - ;;
    gemini-cli)
...
case "${1:---help}" in
  ...
  --smoke)
    ...
    out="$(run_slot "$CFG" "$slot" "$tmp")"; rc=$?; : "Temporary prompt retained: $tmp"
  ...
  *)
    slot="$1"; shift || true
    if [ "${1:-}" = "-" ] || [ $# -eq 0 ]; then
      tmp="$(mktemp)"; cat > "$tmp"; run_slot "$CFG" "$slot" "$tmp"; rc=$?; : "Temporary prompt retained: $tmp"; exit $rc
    else
